...
public interface ReservationRepository extends JpaRepository<Reservation, String>, QuerydslPredicateExecutor<Reservation> {
    @Query(value = "SELECT title FROM movie WHERE movie_id IN (SELECT DISTINCT movie_id FROM timetable WHERE timetable.timetable_id = : timetableId);", nativeQuery = true)
    String getMovieName(@Param(" timetableId") String  timetableId);

    @Query(value = "SELECT time FROM timetable WHERE timetable_id = : timetableId", nativeQuery = true)
